...
// 构建查询语句
$sql = "
SELECT Event.EventID, Event.EventName, Event.Level,
...
if (!empty($search_term)) {
    $sql .= " WHERE Event.EventID LIKE :term 
               OR Event.EventName LIKE :term 
               OR Event.Level LIKE :term
               OR Category.Category_name LIKE :term";
}
...
<div class="app-shell">
    ...
    <section class="app-card">
        <h2 class="card-title">搜索赛事</h2>
        <p class="card-subtitle">支持按照赛事编号、名称、级别或所属大项进行模糊匹配</p>
        <form method="get" class="form-grid">
            <div class="form-group">
                <label for="search">搜索条件</label>
                <input type="text" id="search" name="search" placeholder="如：EVT01、决赛、田径" value="<?= htmlspecialchars($search_term) ?>">
            </div>
            <div class="form-actions">
                <button class="btn btn-secondary" type="submit">开始搜索</button>
                <a class="btn btn-ghost" href="event_list.php">清空筛选</a>
            </div>
        </form>
    </section>

    <section class="app-card">
        <div class="card-title">赛事列表</div>
        ...
        <div class="table-wrapper">
            <table>
                ...
                if ($row_count === 0 && $search_performed) {
                    echo "<tr><td colspan='4' style=\"text-align:center; color:#c0392b;\">未找到匹配的记录</td></tr>";
                }
                ?>
                </tbody>
            </table>
